-- Complete safe pairing-management parity with the legacy controller UI.
-- Deletion remains administrator-only at the edge function and can only run
-- inside the same atomic STOP -> change -> reviewed-state restore chain.

alter table public.project_control_commands
  drop constraint if exists project_control_commands_command_type_check;

alter table public.project_control_commands
  add constraint project_control_commands_command_type_check
  check (
    command_type in (
      'update_pairing',
