...
The overview uses fixed tiles in a compact layout with animated connections to System Health. The layout scales to the available viewport; opening a tile renders the production detail view at its normal size. The preview generates independent synthetic irrigation and moisture histories for progressive drought, cyclic recovery, and heatwave response: Experiment 1 has one graph, Experiment 2 has Control and Drought graphs, and Experiment 3 has four crop/treatment graphs. All three use the production sensor controls, Watering view, and Overlay. The separate calibration tile is retained. Chart traces and sensor swatches use the production portal’s bright pot colors. Regenerate and verify this build when changing shared portal components.
