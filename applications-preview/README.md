...
The overview uses fixed tiles in a compact layout with animated connections to System Health. The layout scales to the available viewport; opening a tile renders the production detail view at its normal size. The preview shares the demo portal’s synthetic irrigation and moisture histories: Experiment 1 has one graph, Experiment 2 has Control and Drought graphs, and Experiment 3 has four crop/treatment graphs. All three use the production sensor controls, Watering view, and Overlay. The separate calibration tile is retained. Preview-only colors use restrained, distinct hues and matching sensor swatches. Regenerate and verify this build when changing shared portal components.
