...
class DemoChannel{handlers:{table:string,handler:Handler}[]=[];timer:ReturnType<typeof setInterval>|null=null;
 ...
 subscribe(){if(!this.timer&&this.handlers.some(h=>h.table==='sensor_readings'))this.timer=setInterval(()=>this.tick(),30000);return this;}
 ...
 close(){if(this.timer)clearInterval(this.timer);this.timer=null;this.handlers=[];}
}
...
export const supabase={
 ...
 from(table:string){let rows:any[]=table==='sensor_readings'?fixture.data.readings:table==='pairings'?fixture.data.pairings:[];const q:any=new Proxy({}, {get(_t,key){if(key==='then')return (resolve:any)=>resolve({data:rows,error:null});if(key==='single'||key==='maybeSingle')return ()=>success(rows[0]||null);return ()=>q;}});return q;},
 auth:{
