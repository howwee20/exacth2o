...
export default defineConfig({root,base:'/demo-app/',publicDir:false,resolve:{alias:[{find:/^\.\/supabase$/,replacement:path.join(root,'offline.ts')},{find:/^react(.*)$/,replacement:path.join(repo,'research-portal/node_modules/react$1')},{find:/^react-dom(.*)$/,replacement:path.join(repo,'research-portal/node_modules/react-dom$1')},{find:/^lucide-react$/,replacement:path.join(repo,'research-portal/node_modules/lucide-react')}],dedupe:['react','react-dom']},plugins:[{
 name:'demo-portal-data',enforce:'pre',transform(code,id){if(id===path.join(repo,'research-portal/src/experimentPresentation.ts'))return code.replace('const groups = new Map<string, PortalExperimentAssignment[]>();',`if (experiment?.id === 'experiment-1' || experiment?.id === 'experiment-2') {
...
const groups = new Map<string, PortalExperimentAssignment[]>();`);if(id===path.join(repo,'research-portal/src/WalkerObservationView.tsx'))return code.replace('Walker Pi 5 Observation','Experiment 3');if(id!==path.join(repo,'research-portal/src/App.tsx'))return;
 ...
 const init={email:'""',authMode:'"sign-in"',inviteToken:'null',sessionReady:'demoSession()',data:'fixture.data',portalAccess:'fixture.access',experimentCatalog:'fixture.experiments',healthSnapshot:'fixture.health',valveEvents:'fixture.valveEvents',runtimeState:'fixture.runtime',configState:'fixture.config'};
 for(const statement of app.body.statements){if(ts.isExpressionStatement(statement)&&ts.isCallExpression(statement.expression)&&statement.expression.expression.getText(source)==='useEffect'&&statement.getText(source).match(/supabase|loadPortalAccess|loadExperimentCatalog|loadHealthSnapshot|loadValveEvents|loadDeviceSyncState|loadSalesSupport|refresh\(/)&&!statement.getText(source).includes('exacth2o-dashboard-live')&&!statement.getText(source).includes('exacth2o-valve-events-live')){edits.push([statement.getStart(source),statement.end,'']);}
 ...
 code=code.replace('{ label: "Checked at the bench", value: <span className="settings-empty-value">Not yet</span> }','{ label: "Pairing status", value: "Verified" }');
